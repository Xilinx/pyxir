...
from pyxir.shared.vector import FloatVector, IntVector
...
def get_layer(BaseLayer):
    # type: (rt_layer.BaseLayer) -> Function

    def _get_layer(X, input_shapes, params, **kwargs):
...
def get_input_layer(InputLayer, ConstantLayer):
    # type: (rt_layer.InputLayer, rt_layer.ConstantLayer) -> Function

    def _get_input_layer(X, input_shapes, params, **kwargs):
        # (XLayer, dict, dict) -> List[rt_layer.RtLayer]
        """
        """
...
def get_constant_layer(ConstantLayer):
    # type: (rt_layer.ConstantLayer) -> Function

    def __get_constant_layer(X, input_shapes, params, **kwargs):
        # (XLayer, dict, dict) -> List[rt_layer.RtLayer]
        """
        """
        assert len(X.bottoms) == 0
...
def get_batchnorm_layer(BatchNormLayer, ConstantLayer, ReluLayer):
    ...
    def __get_batchnorm_layer(X, input_shapes, params, **kwargs):
        # (XLayer, dict, dict) -> List[rt_layer.RtLayer]
        """
...
def get_bias_add_layer(BiasAddLayer, ConstantLayer):
    ...
    def _get_bias_add_layer(X, input_shapes, params, **kwargs):
        # (XLayer, dict, dict) -> List[rt_layer.RtLayer]
...
def get_dense_layer(DenseLayer, ConstantLayer):
    ...
    def _get_dense_layer(X, input_shapes, params, **kwargs):
        # (XLayer, dict, dict) -> List[rt_layer.RtLayer]
        """
...
def get_elementwise_layer(ElementwiseLayer, ReluLayer):
    # type: (rt_layer.TransposeLayer) -> Function

    def _get_elementwise_layer(X, input_shapes, params, **kwargs):
        # (XLayer, dict, dict) -> List[rt_layer.RtLayer]
        """
        """
        # TODO: We are assuming here that the inputs to the elementwise
        #   operation
        #   can't be parameters! This is not the most general assumption!
...
def get_scaling_layer(ScaleLayer, ConstantLayer, ReluLayer):
    ...
    def __get_scaling_layer(X, input_shapes, params, **kwargs):
        # (XLayer, dict, dict) -> List[rt_layer.RtLayer]
        """
        """
        op_name = X.name
...
def get_conv2d_layer(ConvLayer, ConstantLayer):
    ...
    def _get_conv2d_layer(X, input_shapes, params, **kwargs):
        # (XLayer, dict, dict) -> List[rt_layer.RtLayer]
        """
        """
        op_name = X.name
...
def get_conv2d_transpose_layer(Conv2DTransposeLayer, ConstantLayer):
    ...
    def __get_conv2d_transpose_layer(X, input_shapes, params, batch_size=-1,
        ...
        layout = X.attrs['data_layout']
        logger.debug("Conv2DTranspose layout: {}".format(layout))
...
def get_pooling_layer(PoolingLayer):
    ...
    def _get_pooling_layer(X, input_shapes, params, **kwargs):
        # (XLayer, dict, dict) -> List[rt_layer.RtLayer]
        """
        """
        op_name = X.name
        ...
        logger.debug("Pooling layer: {}".format(X.attrs['pool_type']))
        ...
        kernel_h, kernel_w = \
            X.attrs['kernel_size'][0], X.attrs['kernel_size'][1]

        logger.debug("Full paddings: {}".format(full_paddings))
        ...
        x_h = (outsize_h - 1) * strides_h + kernel_h \
            - pad_h_b - pad_h_a - insize_h
        x_w = (outsize_w - 1) * strides_w + kernel_w \
            - pad_w_b - pad_w_a - insize_w

        logger.debug("x_h: {}, x_w: {}".format(x_h, x_w))
        if x_h == 1 or x_w == 1:
            warning = "[WARNING] Incorrect padding values in Pooling layer:" +\
...
def get_quantize_layer(QuantizeLayer):
    ...
    def __get_quantize_layer(X, input_shapes, params, **kwargs):
        # (XLayer, dict, dict) -> List[rt_layer.RtLayer]
        """
...
def get_unquantize_layer(UnQuantizeLayer):
    ...
    def __get_unquantize_layer(X, input_shapes, params, **kwargs):
        # (XLayer, dict, dict) -> List[rt_layer.RtLayer]
        """
        TODO formalize checks
        """
...
def get_quantize_bias_layer(QuantizeBiasLayer):
    ...
    def __get_quantize_bias_layer(X, input_shapes, params, **kwargs):
        # (XLayer, str, dict, dict) -> List[rt_layer.RtLayer]
        """
        TODO formalize checks
        """
...
def get_quantize_inter_layer(QuantizeInterLayer):
    ...
    def __get_quantize_inter_layer(X, input_shapes, params, **kwargs):
        # (XLayer, str, dict, dict) -> List[rt_layer.RtLayer]
        """
        """
        assert len(X.bottoms) == 1
...
def get_quantize_scale_bias_layer(QuantizeScaleBiasLayer):
    ...
    def __get_quantize_scale_bias_layer(X, input_shapes, params, **kwargs):
        # (XLayer, str, dict, dict) -> List[rt_layer.RtLayer]
        """ TODO formalize checks """
        ...
        return [QuantizeScaleBiasLayer(
            ...
            subgraph=X.subgraph,
            input_types=X.attrs['input_types'],  # ['float32']
            # th_scale=X.attrs['quant_params']['th_params'],
            # th_ext=X.attrs['quant_params']['threshold'],
