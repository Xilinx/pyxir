...
from ..x_2_tf_registry import (
    rt_register_xlayer_2_tf,
...
class BiasAddLayer(rt_layer.BaseLayer, RtLayerTF):
    def init(self) -> None:
        ...
        self.bias = tf.compat.v1.placeholder(
            RtLayerTF.dtype_to_tf[self.dtype], shape=self.input_shapes[1]
        )
    ...
    def get_output_tensors(self, inpts: List[tf.Tensor], **kwargs) -> tf.Tensor:
        ...
        if inpt.shape != bias.shape and axis not in [None, -1]:
            shape_for_broadcast = [
                (1 if i != axis else bias.shape[0]) for i in range(len(inpt.shape))
            ]
...
class ConcatLayer(rt_layer.BaseLayer, RtLayerTF):
    def init(self) -> None:
        ...
        for shape in self.input_shapes:
            plch = tf.compat.v1.placeholder(
                RtLayerTF.dtype_to_tf[self.dtype], shape=shape
            )
    ...
    def forward_exec(self, inputs: List[np.ndarray]) -> np.ndarray:
        ...
        with tf.compat.v1.Session() as sess:
            feed_dict = {inpt: inputs[idx] for idx, inpt in enumerate(self.inpts)}
            return sess.run(self.res, feed_dict=feed_dict)
...
class DenseLayer(rt_layer.DenseLayer, RtLayerTF):
    def init(self) -> None:
        ...
        input_shapes, weights, biases = self.input_shapes, self.weights, self.biases
        if weights is not None:
            weights = tf.compat.v1.placeholder_with_default(weights, weights.shape)
            weights = tf.cast(weights, RtLayerTF.dtype_to_tf[self.dtype])
        else:
            weights = tf.compat.v1.placeholder(
                RtLayerTF.dtype_to_tf[self.dtype], shape=input_shapes[1]
            )

        if biases is not None:
            biases = tf.compat.v1.placeholder_with_default(biases, biases.shape)
            biases = tf.cast(biases, RtLayerTF.dtype_to_tf[self.dtype])
        else:
            biases = tf.compat.v1.placeholder(
                RtLayerTF.dtype_to_tf[self.dtype], shape=input_shapes[2]
            )
        self.inpts = [inpt, weights, biases]
        self.res = self.get_output_tensors(self.inpts)[0]
        logger.debug("Dense layer: {}, res shape: {}".format(self.name, self.res.shape))
    ...
    def get_output_tensors(self, inpts: List[tf.Tensor], **kwargs) -> tf.Tensor:
        ...
        dense_name = (
            self.name
            if not self.use_relu and len(self.shape) != 1
            else self.name + "_Dense"
        )
...
class ElementwiseLayer(rt_layer.BaseLayer, RtLayerTF):
    def init(self) -> None:
        ...
        if len(self.inputs) != 2:
            raise ValueError(
                "Run elementwise operation expects 2 inputs, {}"
                " given".format(len(self.inputs))
            )

        if self.op != "Add":
            raise NotImplementedError(
                "Only elementwise add operation"
                " supported at the"
    ...
    def get_output_tensors(
        self, inpts: List[tf.Tensor], override_name=None, **kwargs
    ) -> tf.Tensor:

        assert len(inpts) == 2

        # Do calculation in float32
        inpt0, inpt1 = tf.cast(inpts[0], tf.float32), tf.cast(inpts[1], tf.float32)
...
class ScaleLayer(rt_layer.ScaleLayer, RtLayerTF):
    def init(self) -> None:
        ...
        inpt = tf.compat.v1.placeholder(
            RtLayerTF.dtype_to_tf[self.dtype], shape=input_shapes[0]
        )
    ...
    def get_output_tensors(self, inpts: List[tf.Tensor], **kwargs) -> tf.Tensor:
        ...
        if len(gamma.shape) == 0:
            gamma = (
                ...
                else tf.reshape(gamma, (1,))
            )
            beta = (
                np.reshape(beta, (1,))
                if isinstance(beta, np.ndarray)
                else tf.reshape(beta, (1,))
            )
        ...
        compiler_target = (
            kwargs["compiler_target"] if "compiler_target" in kwargs else None
        )
        is_fused_batch_norm = (
            (self.axis == -1 or self.axis == (len(self.input_shapes[0]) - 1))
            and len(gamma.shape) == 1
            and gamma.shape[0] == self.input_shapes[0][-1]
        )
        ...
        if is_fused_batch_norm:
            ...
            if compiler_target == "DPUv1Compiler":
                ...
                return [tf.add(tf.multiply(inpt, gamma), beta, name=self.name)]
