...
def add(attrs: Dict[str, Any], in_xlayers: List[XLayer]):
    ...
    for ls, rs in zip(reversed(lshape), reversed(rshape)):
        if ls == rs or ls in [1, None] or rs in [1, None]:
            ...
            raise ValueError(
                "Invalid shapes for broadcasted additions:"
...
def bias_add(
    op_name: str, input_layer: XLayer, bias_layer: XLayer, axis: int, **kwargs
):
    """
...
def concat(op_name: str, input_layers: List[XLayer], axis: int, **kwargs):
    ...
    for i in range(len(list(input_layers[0].shapes))):
        ...
        if len(check) > 1 and -1 in check:
            check.remove(-1)
        assert i == axis or len(check) == 1, "i: {0}, axis: {1}, check: {2}".format(
            i, axis, check
        )
...
def dense(
    ...
    **kwargs
    ...
    if "Constant" not in weights_layer.type:
        raise ValueError(
            "Dense layer is expecting a 'Constant' weights layer"
            " as weights input but got layer of type: {}".format(weights_layer.type[0])
        )
...
def multiply(attrs, in_xlayers):
    ...
    for ls, rs in zip(reversed(lshape), reversed(rshape)):
        if ls == rs or ls in [1, None] or rs in [1, None]:
            ...
            raise ValueError(
                "Invalid shapes for broadcasted multiplication:"
                " {} and {}".format(lX.shapes, rX.shapes)
            )
