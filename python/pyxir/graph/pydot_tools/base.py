...
def visualize(pydot_graph, outputfile):
    # type: (pydot.Dot, str) -> pydot.Dot
    """
    Visualize the provided pydot graph
    """
    logger.info("Writing graph visualization to {}".format(outputfile))
    ...
    with open(outputfile, 'wb') as fid:
        ...
        try:
            ...
            logger.error("Graph visualizer failed with exception: {},"
                         " traceback: {}".format(e, traceback.format_exc()))

        fid.close()

        # for n in pydot_graph.get_nodes():
        #     n.set('LayerParameter', Memory[n.get_name()])
    return pydot_graph
...
def remove_node(pydot_graph, node_name, bottom_Xs, top_Xs):
    ...
    for bX in bottom_Xs:
        ...
        for tX in top_Xs:
            ...
            edge = pydot.Edge(b_q, t_q, label="{}->ID->{}".format(bX.name,
                                                                  tX.name))
