...
from ..optimization.xgraph_base_optimizer import XGraphBaseOptimizer
from ..optimization.xgraph_optimization_pass import XGraphOptimizationPass

logger = logging.getLogger("pyxir")

def is_mul_max_leaky_relu_pattern(inX: XLayer, mulX: XLayer, maxX: XLayer) -> bool:
    """
...
class XGraphPatternAnnotator(XGraphVisitor):
    ...
    def visit(self, X: XLayer) -> XLayer:
        ...
        elif 'Maximum' in X.type and\
                any([b in self.lr_layers for b in X.bottoms]):
            if 'patterns' in X.attrs: 
                X.attrs['patterns'].append('LeakyReLU')
            else:
                X.attrs['patterns'] = ['LeakyReLU']
...
class XGraphPatternMutator(XGraphMutator):
    ...
    def visit(self, X: XLayer) -> XLayer:
        if 'Scale' in X.type and 'patterns' in X.attrs and 'LeakyReLU' in X.attrs['patterns']:
            ...
            scale_name = X.bottoms[0] if X.bottoms[0] in self.lr_layers\
                else X.bottoms[1]
            lR = defaultXLayer()
            lR = lR._replace(
                name=X.name,
