...
class XOpRegistry(object):
    ...
    class __XOpRegistry(object):
        ...
        def check_xop(self, xop):
            ...
            if not self.is_xop(xop):
                raise ValueError("Unknown XOp {}, registered XOps are: {}"
                                 .format(device, self.get_xop_names()))

        def is_xop(self, xop):
            # type: (str) -> boolean
            return xop in self.xops

        def get_xop(self, xop):
            # type: (str) -> Device
            self.check_xop(xop)
        ...
        def get_xop_names(self):
            # type: () -> List[str]
            return list(self.xops.keys())

        def register_xop_layout_transform(self,
                                          xop_name,
        ...
        def get_xops_with_transpose_transform(self):
            # type: () -> List[str]
            return [name for name, xop in self.xops.items()
                    if xop.has_transpose_transform()]

        def get_xop_transpose_transform(self, xop):
            # type: (str) -> Function
            """ Return the layout transformation function for the given XOp """
            self.check_xop(xop)
    ...
    def __init__(self):
        ...
        self.__dict__['_XOpRegistry__instance'] = \
            XOpRegistry.__instance
...
def xop_register_op_layout_transform(xop_name):
    # type: (str) -> Function
    """
    ...
    def register_op_layout_transform_decorator(transform_func):
        # type: (Function) -> None
...
def xop_register_op_transpose_transform(xop_name):
    # type: (str) -> Function
    """
    ...
    def register_op_transpose_transform_decorator(transform_func):
        # type: (Function) -> None
