...
class XGraphLayoutTransformationPass(XGraphBasePass):
    ...
    def __init__(self,
                 ...
                 output_png=None):
        super(XGraphLayoutTransformationPass, self).__init__(
            name=name,
            output_png=output_png
        )

        if target_data_layout not in ['NCHW', 'NHWC']:
            raise ValueError("Unsupported target layout for XGraph layout"
                             " transformation pass: {}, only `NCHW` and `NHWC`"
                             " are supported.".format(target_data_layout))
        self.target_data_layout = target_data_layout
        self.target = target

    def execute(self, xgraph, subgraphs_only=False):
        ...
        def transform_layers(bottom_Xs, X, top_Xs):
            ...
            if X.type[0] in layout_transform_ops and\
                ...
                if data_layout == self.target_data_layout:
                    ...
                    logger.debug("Insert bottom transpose: {}, axes: {}"
                                 .format(tb_name, axes_b))

                    # Top transpose
                    axes_t = [self.target_data_layout.index(e)
                              for e in data_layout]
                    ...
                    logger.debug("Insert top transpose: {}, axes: {}"
                                 .format(tt_name, axes_t))
