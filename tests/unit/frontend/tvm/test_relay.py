...
class TestRelayFrontend(unittest.TestCase):
    ...
    def test_conv2d_transpose(self):
        data = relay.var(
            "data",
            relay.TensorType((-1, 1, 3, 3), "float32")
        )
        weight = relay.var("weight")
    ...
    def test_resnet_block(self):
        ...
        max_pool0_expr = relay.nn.max_pool2d(relu0_expr, pool_size=(2, 2),
                                             strides=(2, 2))
        ...
        bias_add0_expr = relay.nn.bias_add(conv2d1_expr, conv2d1_bias,
                                           axis=1)
        relu1_expr = relay.nn.relu(bias_add0_expr)
        add0_expr = relay.op.tensor.add(max_pool0_expr, relu1_expr)

        avg_pool0_expr = relay.nn.avg_pool2d(add0_expr, pool_size=(2, 2),
                                             strides=(2, 2))
    ...
    def test_inception_block(self):
        ...
        concat0_expr = relay.op.tensor.concatenate([relu1_expr, relu2_expr],
                                                   axis=1)
